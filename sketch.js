...
function ball_hit() {
  for(var c=0; c<brickColumns; c++) {
    for(var r=0; r<brickRows; r++) {
      if(bricks[c][r].hidden == 0 && ball_x + ball_radius >= bricks[c][r].x && ball_x - ball_radius <= brickWidth + bricks[c][r].x && ball_y + ball_radius >= bricks[c][r].y - brickHeight && ball_y - ball_radius <= bricks[c][r].y){
        bricks[c][r].hidden = 1;
        score++;
      }
    }
...
function draw() { 
  ...
  if((ball_y + ball_radius >= paddle_y) && (ball_x + ball_radius >= paddle_x) && (ball_x - ball_radius <= paddle_x + paddle_width)){
    ball_dy = -ball_dy;
  }
